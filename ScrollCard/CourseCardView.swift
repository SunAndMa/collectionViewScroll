...
class CourseCardView: UIView {
    ...
    var selectedIndex = 0
    var models: [CourseCard] = [CourseCard]() {
        didSet {
            if models.count < 2 {
    ...
    var selectedCourseClosure: ((CourseCard) -> Void)?
...
extension CourseCardView: UICollectionViewDataSource, UICollectionViewDelegate, CAAnimationDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = CourseCardCell.cellWithCollectionView(collectionView: collectionView, indexPath: indexPath as NSIndexPath, type: .CollectionCellWithNib) as! CourseCardCell
        cell.courseCard = models[indexPath.row]
        return cell
    }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let pointInView = self.convert(collectionView.center, to: collectionView)
        let index = collectionView.indexPathForItem(at: pointInView)?.row ?? 0
        //将最中间的cell放在最上面，所以要先将最后面的提前一层一层执行
